...
Nested 구조는 단일 물리 서버 환경에서 다수의 ESXi 호스트로 구성된 VMware 클러스터 환경을 재현하기 위해 적용하였다. 물리 서버에 직접 설치된 상위 ESXi 호스트는 기반 인프라와 물리 네트워크를 표현하는 역할을 하며, 그 위에 가상머신 형태로 구성된 ESXi 1~4는 실제 개별 물리 서버를 가정한 클러스터 노드로 동작한다. 이를 통해 제한된 하드웨어 환경에서도 vCenter 기반의 통합 관리와 vSAN, vMotion, DRS, HA 등의 클러스터 기능을 구현할 수 있도록 하였다.
